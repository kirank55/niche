...
description: >
  ...
  pass keep-gate (literal quotes, four SaaS search classes, no occupied
  bundle, not a native SoR module, Occupied plugin is file_on not KEEP,
  hook-only leftover is not vacancy). Use when asked to find, hunt, or propose
...
- `as_company` **defaults to Occupied or `file_on`**. A Sparse/Greenfield company keep is fail-closed and requires `keep_gate: pass`. Fill `keep_gate_checks` (G1-G10) on the card first; a missing key is fail. Do not set `keep_gate: pass` from leftover prose.
- `as_plugin` Occupied or Saturated **forbids** `as_company` Sparse/Greenfield. That split already says the leftover is a flag/plugin/issue: emit `file_on`, not KEEP.
- Leftover that is only a different hook than v1 named (tray vs native window, "not write-time", "does not intercept", "only when invoked via CLI") is Occupied / `file_on`. `wrong_substrate` is SKU mismatch only (Clio vs MyCase), not hook mismatch.
...
7. **Seat-match + dual-score.** Load [calibration-saas.md](references/calibration-saas.md) (before the first 0-10), then [seat-match.md](references/seat-match.md), then [rubric.md](references/rubric.md). Apply [gate-bind-saas.md](references/gate-bind-saas.md) seat-match notes: SKU mismatch is `wrong_substrate`, not `language_scoped`; **hook mismatch is not `wrong_substrate`**; fetched G2/Capterra listings are `adjacent_pain`, never `exact`; a dedicated native module for the step is `exact`. One label per incumbent; strictest mismatch wins. `problem_density` counts `adjacent_pain`; `exact_mechanics_density` counts `exact` only and must be <= steelman ceiling. Do not label a row `adjacent_pain` only so `exact_mechanics_density` can stay 0.

8. **Gates.** Apply auto-rejects and falsification from [rubric.md](references/rubric.md) **as remapped in** [gate-bind-saas.md](references/gate-bind-saas.md). Load gate-bind only (it is the keep latch). Fill every `keep_gate_checks` key (G1-G10) before setting `keep_gate`. G2 classes are those in [search-playbook-saas.md](references/search-playbook-saas.md). A Sparse/Greenfield `as_company` keep requires `keep_gate: pass`. On `keep_gate: fail`, emit Occupied / `file_on` / `drop` as the company verdict even if leftover prose is eloquent. Occupied plugin, hook-only leftover, or `file_on: none` with a named host are not close calls: they are fail.

9. **Emit** the candidate card from [output-template-saas.md](references/output-template-saas.md), including the `niche` block, `slices`, `search_classes`, `keep_gate_checks`, and `keep_gate`. Split `as_company` / `as_oss` / `as_plugin`. Set `file_on` when the honest leftover is a flag, marketplace issue, plugin, or native module on a named host. Do not emit a headline rater decision (that belongs to a rater, not the finder). Finder disposition is the split verdicts plus `file_on` plus `claim_hygiene` plus `keep_gate`. Do not emit this card when step 2 stopped on `intake_blocked`. Do not emit `as_company` Sparse/Greenfield when `keep_gate` is `fail`. Do not count a tracker bullet as a keep unless this card was emitted.
