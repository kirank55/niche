...
Occupied plugin plus `file_on: none` is a card fail. Occupied or
Saturated plugin **forbids** Sparse/Greenfield company: that split is
`file_on`, not KEEP.
...
manual is not a vacant process. Labeling a row `adjacent_pain` or
`wrong_substrate` because leftover is only a different hook than v1
named is not a vacant process (G10).
...
- `keep` -- `as_company` is Sparse or Greenfield, no auto-reject, all
  ...
  "could not find 5" plus why), **and** `keep_gate: pass` with every
  `keep_gate_checks` key `pass`. A tracker bullet without an emitted
  candidate card is not a keep. `as_plugin` Occupied/Saturated is not
  a company keep.
